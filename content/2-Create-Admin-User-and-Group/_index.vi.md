...
> **Lưu ý:** Trước khi vào bài làm chúng ta cần chọn khu vực để thực hiện bài làm thực hành, ở đây chúng ta nên chọn khu vực sigapore (vì sẽ giúp chúng ta trong quá trình làm bài mượt và không bị lag )

   ![AWS IAM](/images/01/11002.png?featherlight=false&width=90pc)

2. Nhấn vào thanh tìm kiếm trên aws vào tìm kiếm **IAM Manage acces to AWS resources**

   ![AWS IAM](/images/01/11001.png?featherlight=false&width=90pc)

3. Ở cửa sổ bên trái nhấn chọn **User group** sau đó chọn tiếp **create group**

   ![AWS IAM](/images/01/11003.png?featherlight=false&width=90pc)

4. Dưới mục **User group name**, nhập tên Group (Ví dụ: *AdminGroup, GroupAdmin, AdminAWS,...*) và cuộn chuột xuống dưới

   ![AWS IAM](/images/01/11004.png?featherlight=false&width=90pc)

5. Ở phần **Attach permissions policies - Optional**, gõ **AdministratorAccesss** vào thanh tìm kiếm (như bước 1) và nhấn chọn **AdministratorAccesss**. Cuối cùng, chọn **Create user Group** (ở bước 2).
...
1. Ở thanh điều hướng bên trái, chọn **Users** sau đó chọn **Add User**

   ![AWS IAM](/images/02/11007.png?featherlight=false&width=90pc)

2. Nhập tên User (Ví dụ: *AdminUser*).
    + Click **Provide user access to the AWS Management Console - optional** *(Như hình ở bước 2)*
    + Sẽ xuất hiện 1 cửa sổ mới và nhấn chọn **i want to create an IAM user***(Như hình ở bước 3)*
    + Click **Custom password** rồi gõ một password tùy ý của bạn (lưu ý: bạn phải ghi nhớ mật khẩu này cho những lần đăng nhập trong tương lai) Hoặc có thể để ngẫu nhiên mật khẩu *(như trong hình để mặc định)*
...
7. Sau khi hoàn thành tạo user. Có thể download.csv để lưu trữ Access key.
   ![AWS IAM](/images/02/11011.png?featherlight=false&width=90pc)
   *(Có thể download Access key khi tạo **Admin user** xong)*
...
# Đăng nhập vào Admin User

1. Trở về cửa sổ chính của AWS, sau đó chọn lại vào dịch vụ IAM, và chọn **Users** ở thanh bên trái.
...
>**Gợi ý:** *Nếu các bạn quên tài khoản mật khẩu có thể vào lại file exel nãy tải về có tên là **download.csv** ở phần tạo IAM user*

   ![AWS IAM](/images/03/11017.png?featherlight=false&width=90pc)
   

6. Sau khi truy cập thành công vào tài khoản của bạn dưới danh nghĩa của một IAM User **AdminUser**.
